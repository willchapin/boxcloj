(ns boxcloj.core
  ...
  (:require-macros [boxcloj.macros :refer [my_set!]]))
...
(defn setAnimFrame []
  (my_set! js/window .-requestAnimFrame (getAnimFrameType)))
...
(defn draw-all! [nodes]
  (loop [node (first nodes) nodes (rest nodes)]
    (when node
      (if (selected? node) (def color "#444")
...
(defn draw! [color [x y r]]
  ...
  ; (my_set! ctx .-fillStyle color .-lineWidth 1)
   (set! (. ctx -lineWidth) 1)
